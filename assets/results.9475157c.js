import"./modulepreload-polyfill.c7c6310f.js";const f=()=>{document.querySelector("#app").innerHTML=`
  ...
  border: 1px solid #FFFFFF`,m=L.divIcon({className:"my-custom-pin",iconAnchor:[0,24],labelAnchor:[-6,0],popupAnchor:[0,-36],html:`<span style="${p}" />`});L.marker([o.coordinates.latitude,o.coordinates.longitude],{icon:m}).addTo(a).bindPopup(`
        <h3>${o.name}</h3>
        <img src="${o.image_url}" />
        <p>${o.display_phone}</p>
        <a href="${o.url}">More Info...</a>
    `)}};function d(e,t){let o=`https://www.apitutor.org/yelp/v3/businesses/search?location=${t}&categories=${e}&limit=50&term=black owned`;e=="museums"&&(o=`https://www.apitutor.org/yelp/v3/businesses/search?location=${t}&categories=${e}&limit=50`),console.log(o),fetch(o).then(s=>s.json()).then(s=>{s.businesses.length==0?v():(g(s),h(s))})}const g=e=>{console.log(e);const t=e.businesses.map(o=>(console.log(o.name,o.rating),b(o)));document.querySelector(".list").innerHTML=t.join("")};function v(){const e=`
    <section id="bg">
      <div>
  ...
  `;document.body.insertAdjacentHTML("afterbegin",e),document.querySelector("#close").onclick=y}function y(){document.querySelector("#bg").remove()}function b(e){let t="result";return r.includes(e.id)&&(t="result black-owned"),`
  <section class="${t}">
     <img src= "${e.image_url}"/>
     <div> 
          <h3>${e.name} </h3>
 ...
 `}function u(){const e=document.querySelector("#category").value,t=document.querySelector("#location").value;d(e,t)}f();document.querySelector("#location").addEventListener("change",u);document.querySelector("#category").addEventListener("change",u);const n=new Proxy(new URLSearchParams(window.location.search),{get:(e,t)=>e.get(t)});let c="Phoenix, Az",i="";n.city&&(c=n.city);n.category&&(i=n.category);n.city;console.log(n.city);console.log(n.category);document.querySelector("#location").value=c;const l=document.querySelectorAll("#category option");i=="restaurant"&&(l[1].selected="selected");i=="events"&&(l[2].selected="selected");i=="coffeeShop"&&(l[3].selected="selected");i=="healthBeauty"&&(l[4].selected="selected");i=="nightlife"&&(l[5].selected="selected");i=="museums"&&(l[6].selected="selected");c=document.querySelector("#location").value;i=document.querySelector("#category").value;console.log(i,c);d(i,c);
